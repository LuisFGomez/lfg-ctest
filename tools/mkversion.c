 ...
 *     #endif
 *
 * Runs: `git -C <source_dir> describe --match 'v*'`
 * Parses: `v<major>.<minor>[-<patch>-g<hash>]`
 *
 * Fallback when git or a matching tag is unavailable: emits 0.0.0.
 *
...
main(int argc, char **argv)
{
    ...
    /* Compose `git -C <dir> describe ...`. The source dir is trusted
     * (supplied by the build system), so a plain double-quoted splice
     * is sufficient; bizarre paths are the caller's problem. */
