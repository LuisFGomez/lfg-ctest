...
| `lfg_ct_format_id(buf, cap, file, suite, test)` | Render an entry id into `buf` exactly as `--list` emits it. `test = NULL` renders a suite id. Truncates silently; returns the length the id needed, so `result >= cap` means truncation. |
